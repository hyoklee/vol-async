...
int main(int argc, char *argv[]) {
  hid_t file_id = -1, grp_id = -1, grp1_id = -1, dset1_id = -1, dset0_id = -1,
        dspace_id = -1, attr_space = -1, attr0 = -1, attr1 = -1;
  const char *file_name = "async_test_serial.h5";
  const char *grp_name = "Group";
  int *data0_write = NULL, *data0_read = NULL, attr_data0, attr_data1,
      attr_read_data0 = 0, attr_read_data1 = 0;
  int i, ret = 0;
  ...
  file_id =
      H5Fcreate_async(file_name, H5F_ACC_TRUNC, H5P_DEFAULT, async_fapl, es_id);
  if (file_id < 0) {
  ...
  fflush(stdout);
  grp_id = H5Gcreate_async(file_id, grp_name, H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT, es_id);
  ...
  grp1_id = H5Gcreate_async(file_id, grp_name, H5P_DEFAULT, H5P_DEFAULT,
                            H5P_DEFAULT, es_id);
  if (grp1_id < 0) {
    fprintf(stderr, "Error with group create\n");
    ret = -1;
  ...
  if (strcmp("loc_id=0x100000000000000 (file), name=\"Group\", "
             "lcpl_id=H5P_DEFAULT, gcpl_id=H5P_DEFAULT, "
  ...
  if (strcmp("async_test_serial_event_set_error_stack.c",
             err_info.app_file_name)) {
    fprintf(stderr,
            "Event set didn't return app source file name correctly?!?\n");
    ret = -1;
    goto done;
  }
  ...
  if (strcmp("main", err_info.app_func_name)) {
    fprintf(stderr,
            "Event set didn't return app source function name correctly?!?\n");
    ret = -1;
    goto done;
  }
  ...
  if (err_line != err_info.app_line_num) {
    fprintf(stderr,
            "Event set didn't return app source line # correctly?!?, got: %u\n",
  ...
  if (print_dbg_msg)
    fprintf(stderr, "H5Dcreate 1 start (should fail as using previous event "
                    "set with failed op)\n");
  fflush(stdout);
  H5E_BEGIN_TRY {
    dset1_id = H5Dcreate_async(grp1_id, "dset1", H5T_NATIVE_INT, dspace_id,
                               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id);
  }
  H5E_END_TRY
  if (dset1_id >= 0) {
    fprintf(stderr,
            "Should not be able to add task to an event set with failed ops\n");
    ret = -1;
    goto done;
  }
  if (print_dbg_msg)
  ...
  fflush(stdout);
  dset0_id = H5Dcreate_async(grp_id, "dset0", H5T_NATIVE_INT, dspace_id,
                             H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es1_id);
  if (dset0_id < 0) {
  ...
  attr0 = H5Acreate_async(dset0_id, "attr_0", H5T_NATIVE_INT, attr_space,
                          H5P_DEFAULT, H5P_DEFAULT, es1_id);
  if (attr0 < 0) {
    fprintf(stderr, "First attribute create failed\n");
    ret = -1;
    goto done;
  }
  attr1 = H5Acreate_async(dset0_id, "attr_1", H5T_NATIVE_INT, attr_space,
                          H5P_DEFAULT, H5P_DEFAULT, es1_id);
  if (attr1 < 0) {
  ...
  fflush(stdout);
  status = H5Dwrite_async(dset0_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                          H5P_DEFAULT, data0_write, es1_id);
  if (status < 0) {
  ...
  fflush(stdout);
  status = H5Dread_async(dset0_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                         H5P_DEFAULT, data0_read, es1_id);
  if (status < 0) {
