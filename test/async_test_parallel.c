...
int main(int argc, char *argv[]) {
   ...
   * offset[0], offset[1], my_size[0], my_size[1]); */

  // W0, R0, W1, R1, W1', W0'
  status = H5Dwrite(dset0_id, H5T_NATIVE_INT, mspace_id, fspace_id, dxpl_id,
                    data0_write);
  if (status < 0) {
  ...
  status =
      H5Dread(dset0_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id, data0_read);
  if (status < 0) {
  ...
  status = H5Dwrite(dset1_id, H5T_NATIVE_INT, mspace_id, fspace_id, dxpl_id,
                    data1_write);
  if (status < 0) {
  ...
  status =
      H5Dread(dset1_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id, data1_read);
  if (status < 0) {
  ...
  status = H5Dwrite(dset0_id, H5T_NATIVE_INT, mspace_id, fspace_id, H5P_DEFAULT,
                    data0_write);
  if (status < 0) {
    fprintf(stderr, "Error with dset 0 write\n");
    ret = -1;
