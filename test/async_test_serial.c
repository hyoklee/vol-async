...
static int link_iterate_cb(hid_t group_id, const char *link_name,
                           const H5L_info2_t *info, void *_op_data) {
...
int main(int argc, char *argv[]) {
  ...
  int *data0_write = NULL, *data0_read = NULL, *data1_write = NULL,
      *data1_read = NULL;
  ...
  fflush(stdout);
  dset0_id = H5Dcreate(grp_id, "dset0", H5T_NATIVE_INT, dspace_id, H5P_DEFAULT,
  ...
  fflush(stdout);
  dset1_id = H5Dcreate(grp_id, "dset1", H5T_NATIVE_INT, dspace_id, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  if (dset1_id < 0) {
  ...
  attr0 = H5Acreate(dset0_id, "attr_0", H5T_NATIVE_INT, attr_space, H5P_DEFAULT,
                    H5P_DEFAULT);
  attr1 = H5Acreate(dset1_id, "attr_1", H5T_NATIVE_INT, attr_space, H5P_DEFAULT,
                    H5P_DEFAULT);
  if (print_dbg_msg)
  ...
  fflush(stdout);
  status = H5Dwrite(dset0_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, async_dxpl,
                    data0_write);
  if (status < 0) {
  ...
  fflush(stdout);
  status = H5Dread(dset0_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, async_dxpl,
                   data0_read);
  if (status < 0) {
  ...
  fflush(stdout);
  /* usleep(sleeptime); */
  status = H5Dwrite(dset1_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, async_dxpl,
                    data1_write);
  if (status < 0) {
  ...
  status = H5Dread(dset1_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, async_dxpl,
                   data1_read);
  if (status < 0) {
  ...
  status = H5Literate2(grp_id, H5_INDEX_NAME, H5_ITER_INC, &idx,
                       link_iterate_cb, &nlink);
  if (status < 0) {
