...
int main(int argc, char *argv[]) {
  hid_t file_id, grp_id, dset1_id, dset0_id, fspace_id, mspace_id, dxpl_col_id,
      dxpl_ind_id;
  ...
  dset0_id = H5Dcreate(grp_id, "dset0", H5T_NATIVE_INT, fspace_id, H5P_DEFAULT,
  ...
  dset1_id = H5Dcreate(grp_id, "dset1", H5T_NATIVE_INT, fspace_id, H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT);
  if (dset1_id < 0) {
  ...
  // CW0
  status = H5Dwrite(dset0_id, H5T_NATIVE_INT, mspace_id, fspace_id, dxpl_col_id,
                    data0_write);
  if (status < 0) {
  ...
  // CW1
  status = H5Dwrite(dset1_id, H5T_NATIVE_INT, mspace_id, fspace_id, dxpl_col_id,
                    data1_write);
  if (status < 0) {
  ...
  // CR0
  status = H5Dread(dset0_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_col_id,
                   data0_read);
  if (status < 0) {
  ...
  // W0
  status = H5Dwrite(dset0_id, H5T_NATIVE_INT, mspace_id, fspace_id, dxpl_ind_id,
                    data0_write);
  if (status < 0) {
    fprintf(stderr, "Error with W0\n");
    ret = -1;
  ...
  // CR1
  status = H5Dread(dset1_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_col_id,
                   data1_read);
  if (status < 0) {
  ...
  // W1
  status = H5Dwrite(dset1_id, H5T_NATIVE_INT, mspace_id, fspace_id, dxpl_ind_id,
                    data1_write);
  if (status < 0) {
    fprintf(stderr, "Error with W1\n");
    ret = -1;
