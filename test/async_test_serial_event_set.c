...
int main(int argc, char *argv[]) {
  hid_t file_id, grp_id, dset1_id, dset0_id, dspace_id, async_dxpl, attr_space,
      attr0, attr1;
  const char *file_name = "async_test_serial.h5";
  const char *grp_name = "Group";
  int *data0_write, *data0_read, *data1_write, *data1_read, attr_data0,
      attr_data1, attr_read_data0 = 0, attr_read_data1 = 0;
  int i, ret = 0;
  ...
  fflush(stdout);
  /* grp_id = H5Gcreate(file_id, grp_name, H5P_DEFAULT, H5P_DEFAULT,
   * H5P_DEFAULT); */
   ...
   */
  dset1_id = H5Dcreate_async(grp_id, "dset1", H5T_NATIVE_INT, dspace_id,
                             H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT, es_id);
  if (dset1_id < 0) {
  ...
  attr1 = H5Acreate_async(dset1_id, "attr_1", H5T_NATIVE_INT, attr_space,
                          H5P_DEFAULT, H5P_DEFAULT, es_id);
  ...
  fflush(stdout);
  status = H5Dread_async(dset0_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, async_dxpl,
                         data0_read, es_id);
  if (status < 0) {
  ...
  fflush(stdout);
  status = H5Dwrite_async(dset1_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                          async_dxpl, data1_write, es_id);
  if (status < 0) {
  ...
  status = H5Dread_async(dset1_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, async_dxpl,
                         data1_read, es_id);
  if (status < 0) {
