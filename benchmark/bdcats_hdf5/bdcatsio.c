...
void read_h5_data(int rank, hid_t loc, hid_t filespace, hid_t memspace) {
  ...
  ierr =
      H5Dread(dset_id, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT, x);
  H5Dclose(dset_id);
  // if (rank == 0) printf ("Read variable 1 \n");

  dset_id = H5Dopen2(loc, "y", dapl);
  ierr =
      H5Dread(dset_id, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT, y);
  H5Dclose(dset_id);

  dset_id = H5Dopen2(loc, "z", dapl);
  ierr =
      H5Dread(dset_id, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT, z);
  H5Dclose(dset_id);

  dset_id = H5Dopen2(loc, "id1", dapl);
  ierr =
      H5Dread(dset_id, H5T_NATIVE_INT, memspace, filespace, H5P_DEFAULT, id1);
  H5Dclose(dset_id);

  dset_id = H5Dopen2(loc, "id2", dapl);
  ierr =
      H5Dread(dset_id, H5T_NATIVE_INT, memspace, filespace, H5P_DEFAULT, id2);
  H5Dclose(dset_id);

  dset_id = H5Dopen2(loc, "px", dapl);
  ierr =
      H5Dread(dset_id, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT, px);
  H5Dclose(dset_id);

  dset_id = H5Dopen2(loc, "py", dapl);
  ierr =
      H5Dread(dset_id, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT, py);
  H5Dclose(dset_id);

  dset_id = H5Dopen2(loc, "pz", dapl);
  ierr =
      H5Dread(dset_id, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT, pz);
  H5Dclose(dset_id);
