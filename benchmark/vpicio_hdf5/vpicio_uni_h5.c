...
void create_and_write_synthetic_h5_data(int rank, hid_t loc, hid_t *dset_ids,
  ...
  dset_ids[0] = H5Dcreate(loc, "x", H5T_NATIVE_FLOAT, filespace, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
  dset_ids[1] = H5Dcreate(loc, "y", H5T_NATIVE_FLOAT, filespace, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
  dset_ids[2] = H5Dcreate(loc, "z", H5T_NATIVE_FLOAT, filespace, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
  dset_ids[3] = H5Dcreate(loc, "id1", H5T_NATIVE_INT, filespace, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
  dset_ids[4] = H5Dcreate(loc, "id2", H5T_NATIVE_INT, filespace, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
  dset_ids[5] = H5Dcreate(loc, "px", H5T_NATIVE_FLOAT, filespace, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
  dset_ids[6] = H5Dcreate(loc, "py", H5T_NATIVE_FLOAT, filespace, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);
  dset_ids[7] = H5Dcreate(loc, "pz", H5T_NATIVE_FLOAT, filespace, H5P_DEFAULT,
                          H5P_DEFAULT, H5P_DEFAULT);

  ierr =
      H5Dwrite(dset_ids[0], H5T_NATIVE_FLOAT, memspace, filespace, plist_id, x);
  /* if (rank == 0) printf ("Written variable 1 \n"); */

  ierr =
      H5Dwrite(dset_ids[1], H5T_NATIVE_FLOAT, memspace, filespace, plist_id, y);
  /* if (rank == 0) printf ("Written variable 2 \n"); */

  ierr =
      H5Dwrite(dset_ids[2], H5T_NATIVE_FLOAT, memspace, filespace, plist_id, z);
  /* if (rank == 0) printf ("Written variable 3 \n"); */

  ierr =
      H5Dwrite(dset_ids[3], H5T_NATIVE_INT, memspace, filespace, plist_id, id1);
  /* if (rank == 0) printf ("Written variable 4 \n"); */

  ierr =
      H5Dwrite(dset_ids[4], H5T_NATIVE_INT, memspace, filespace, plist_id, id2);
  /* if (rank == 0) printf ("Written variable 5 \n"); */

  ierr = H5Dwrite(dset_ids[5], H5T_NATIVE_FLOAT, memspace, filespace, plist_id,
                  px);
  /* if (rank == 0) printf ("Written variable 6 \n"); */

  ierr = H5Dwrite(dset_ids[6], H5T_NATIVE_FLOAT, memspace, filespace, plist_id,
                  py);
  /* if (rank == 0) printf ("Written variable 7 \n"); */

  ierr = H5Dwrite(dset_ids[7], H5T_NATIVE_FLOAT, memspace, filespace, plist_id,
                  pz);
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < nts; i++) {
    ...
    grp_ids[i] =
        H5Gcreate(file_id, grp_name, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    timer_on(2);
    timer_on(3);
    create_and_write_synthetic_h5_data(my_rank, grp_ids[i], dset_ids[i],
                                       filespace, memspace, dxpl);
    timer_off(2);
    if (my_rank == 0)
    ...
    if (i != nts - 1) {
      if (my_rank == 0) {
        printf("  sleep for %ds start\n", sleep_time);
        fflush(stdout);
      }
      if (sleep_time > 0)
        sleep(sleep_time);
      if (my_rank == 0) {
        printf("  sleep for %ds end\n", sleep_time);
        fflush(stdout);
      }
    }
