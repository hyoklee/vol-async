...
void print_data(int n) {
  int i;
  for (i = 0; i < n; i++)
    printf("%f %f %f %d %d %f %f %f\n", x[i], y[i], z[i], id1[i], id2[i], px[i],
           py[i], pz[i]);
}

// Create HDF5 file and read data
void read_h5_data(int rank, hid_t loc, hid_t *dset_ids, hid_t filespace,
                  hid_t memspace, hid_t dxpl, hid_t es_id) {
  hid_t dapl;
  ...
  ierr = H5Dread_async(dset_ids[0], H5T_NATIVE_FLOAT, memspace, filespace, dxpl,
                       x, es_id);

  ierr = H5Dread_async(dset_ids[1], H5T_NATIVE_FLOAT, memspace, filespace, dxpl,
                       y, es_id);

  ierr = H5Dread_async(dset_ids[2], H5T_NATIVE_FLOAT, memspace, filespace, dxpl,
                       z, es_id);

  ierr = H5Dread_async(dset_ids[3], H5T_NATIVE_INT, memspace, filespace, dxpl,
                       id1, es_id);

  ierr = H5Dread_async(dset_ids[4], H5T_NATIVE_INT, memspace, filespace, dxpl,
                       id2, es_id);

  ierr = H5Dread_async(dset_ids[5], H5T_NATIVE_FLOAT, memspace, filespace, dxpl,
                       px, es_id);

  ierr = H5Dread_async(dset_ids[6], H5T_NATIVE_FLOAT, memspace, filespace, dxpl,
                       py, es_id);

  ierr = H5Dread_async(dset_ids[7], H5T_NATIVE_FLOAT, memspace, filespace, dxpl,
                       pz, es_id);
...
int main(int argc, char *argv[]) {
  ...
  if (my_rank == 0)
    printf("Reading %s ... \n", grp_name);
  read_h5_data(my_rank, grp_ids[i], dset_ids[i], filespace, memspace,
               async_dxpl, es_id);
