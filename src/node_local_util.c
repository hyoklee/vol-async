...
mmap_file *mmap_attach_fd(size_t init_size, int fd) { return NULL; }

mmap_file *mmap_setup_newfile(size_t init_size, char *file_path,
                              int open_flags) {
...
int mmap_cpy(mmap_file *mmf, void *src, size_t offset, size_t len) {
  ...
  if (offset + len > mmf->current_size) {
    printf("Attempting offset + len > map size. It must be less than %zu "
           "bytes. Please call "
           "mmap_extend() first.\n",
           mmf->current_size);
    return -1;
...
size_t mmap_extend(mmap_file *mmf, size_t addition_size) {
  assert(mmf && mmf->map);
  int page_size = getpagesize();
  assert(mmf->current_size > 0 && mmf->current_size % page_size == 0 &&
         addition_size > 0);
  ...
  if (addition_size % page_size == 0)
    ...
    new_map_size =
        mmf->current_size + (addition_size / page_size + 1) * page_size;
